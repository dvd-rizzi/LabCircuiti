...
void voltage_function(parameters p) {
    ...
    c1->SetFillColor(0);
    c1->SetGrid();
    TF1 *v_function = new TF1("v_function","5*([0]*sqrt((1-(pow(2*pi*x,2))*[1]*[2])^2 + (2*pi*x*[2]*[3])^2))/sqrt((([4]+[0])*(1-(pow(2*pi*x,2))*[1]*[2]) + [3])^2 + (2*pi*x*([1]+[2]*[3]*([4]+[0])))^2)", 1000, 15000);
...
void response_function(parameters p) {
    TCanvas *c2 = new TCanvas("c1", "response_function", 1280, 800);
    c2->SetFillColor(0);
    c2->SetGrid();
    TF1 *v_function = new TF1("R_function","([0]*sqrt((1-(pow(2*pi*x,2))*[1]*[2])^2 + (2*pi*x*[2]*[3])^2))/sqrt((([4]+[0])*(1-(pow(2*pi*x,2))*[1]*[2]) + [3])^2 + (2*pi*x*([1]+[2]*[3]*([4]+[0])))^2)", 1000, 15000);
    v_function->SetParameter(0, p.R);
    v_function->SetParameter(1, p.L);
    v_function->SetParameter(2, p.C);
    v_function->SetParameter(3, p.R_L);
